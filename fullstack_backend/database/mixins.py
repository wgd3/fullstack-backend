...
class PaginatedAPIMixin(object):
    """
    Mixin class intended to be used with any model that can be returned as part
    of a collection.

    Usage::
        >>> class Post(Model, PaginatedAPIMixin):
        >>>     pass
        >>> Post.to_collection_dict(Post.query.all(), 1, 10, 'posts')
        >>> {
        >>>   "items": [],
        >>>   "_meta": {
        >>>     "page": 1,
    ...
    def to_collection_dict(query, page, per_page, endpoint, **kwargs):
        resources: Pagination = query.paginate(page, per_page, False)
        data = {
            ...
            "_links": {
                ...
                "next": url_for(
                    endpoint, page=resources.next_num, per_page=per_page, **kwargs
                )
                if resources.has_next
                else None,
                "prev": url_for(
                    endpoint, page=resources.prev_num, per_page=per_page, **kwargs
                )
